...
main(void)
{
  ...
  if (!wait_for_network()){
      std::cerr << connections::Communication << ": Error: Wait for a network failed!"
                << std::endl;
      return EXIT_FAILURE;
  } else {
      std::cout << connections::Communication << ": Network is accessible!" << std::endl;
  }
  
  if (!helper::mountFolder("romfs", consts::DocumentRoot))
  {
      std::cerr << connections::Communication << ": Error: Unable to mount folders!" << std::endl;
  } else {
    std::cout << connections::Communication << ": Mount of romfs is successful!" << std::endl;
  }
