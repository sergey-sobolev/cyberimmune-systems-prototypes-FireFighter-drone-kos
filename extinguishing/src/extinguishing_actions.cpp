...
static nk_err_t
StartActionImpl(
  struct ffd_ExtinguishingActions* self,
  ...
  __rtl_unused nk_arena* res_arena)
{
  ...
  std::cerr << connections::Extinguishing << ": Started StartAction"
            << std::endl;
  impl->handler->StartAction();
  std::cerr << connections::Extinguishing << ": Stopped StartAction"
            << std::endl;
               ...
               __rtl_unused nk_arena* res_arena)
{
  ...
  std::cerr << connections::Extinguishing << ": Started StopAction"
            << std::endl;
  impl->handler->StopAction();
  std::cerr << connections::Extinguishing << ": Stopped StopAction"
            << std::endl;

  return NK_EOK;
}

ffd_ExtinguishingActions*
ExtinguishingActionsHandler::CreateImpl()
{
...
// ExtinguishingActionsHandler::ExtinguishingActionsHandler(rtl_uint32_t
// coordinates)
//{}
void
ExtinguishingActionsHandler::StartAction()
{
...
void
ExtinguishingActionsHandler::StopAction()
{
  // extinguishing takes 5 seconds
  std::this_thread::sleep_for(5s);
}
...
void
ExtinguishingActionsHandler::ProceedAction()
{
  // extinguishing takes 5 seconds
  std::this_thread::sleep_for(5s);
}
