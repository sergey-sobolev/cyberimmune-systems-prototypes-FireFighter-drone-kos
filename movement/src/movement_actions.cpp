           ...
           __rtl_unused nk_arena* res_arena)
{
  ...
  std::cout << connections::Movement << ": Started MoveTo: " << req->task
            << std::endl;
  impl->handler->MoveTo((int)req->task);
  std::cout << connections::Movement << ": Stopped  MoveTo: " << req->task
            << std::endl;
...
void
MovementActionsHandler::MoveTo(rtl_uint32_t task)
{
