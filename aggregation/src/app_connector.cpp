...
AppConnector::Connect()
{
...

  {
    ...
    ffd_NavigationCoordinates_proxy_init(
      &m_navigationCoordinatesProxy, &m_navigationTransport.base, riid);
  }
...
AppConnector::Get()
{
  ...
  if (code == NK_EOK) {
    ...
    std::cerr << connections::Aggregation << " -> " << connections::Navigation
              << ": coordinates(), result: " << (int)res.coordinates
              << std::endl;
  }
