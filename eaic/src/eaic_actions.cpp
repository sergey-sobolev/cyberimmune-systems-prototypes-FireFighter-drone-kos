                  ...
                  __rtl_unused nk_arena* res_arena)
{
  ...
  std::cerr << connections::EAIC << ": Started StartActionAt(" << req->task
            << ")" << std::endl;
  ...
  std::cerr << connections::EAIC << ": Stopped  StartActionAt" << std::endl;
  return NK_EOK;
}
                  ...
                  __rtl_unused nk_arena* res_arena)
{
  ...
  std::cerr << connections::EAIC << ": Started ConfirmAction" << std::endl;
  // if (req->task == authenticated_task) {

  // impl->handler->StartAt(req->task);

  std::cerr << connections::EAIC << ": Stopped  ConfirmAction" << std::endl;
  // res->result = 1;
  //} else {
  // res->result = 0;
  //}
...
{
  EAICActionsHandlerImpl* impl = (EAICActionsHandlerImpl*)self;
  std::cerr << connections::EAIC << ": Started StopAction" << std::endl;
  // if (req->task == authenticated_task) {

  // impl->handler->StartAt(req->task);

  std::cerr << connections::EAIC << ": Stopped  StopAction" << std::endl;
  // res->result = 1;
  //} else {
  // res->result = 0;
  //}
...
ffd_EAICActions*
EAICActionsHandler::CreateImpl(AppConnectorPtr connector)
{
...
EAICActionsHandler::EAICActionsHandler(AppConnectorPtr connector)
  : appCon(connector)
{
}
